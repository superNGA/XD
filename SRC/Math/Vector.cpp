...
int screenWidth = COMMON_STORAGE::CHEAT_STATE::SCREEN_WIDTH;
//int screenWidth = GetSystemMetrics(SM_CXSCREEN); // 0
//int screenHeight = GetSystemMetrics(SM_CYSCREEN); // 1
int screenHeight = COMMON_STORAGE::CHEAT_STATE::SCREEN_HEIGHT;

void UPDATE_WORLD_TO_SCREEN_RESOLUTION()
{
    screenWidth = COMMON_STORAGE::CHEAT_STATE::SCREEN_WIDTH;
    screenHeight = COMMON_STORAGE::CHEAT_STATE::SCREEN_HEIGHT;
}
...
//    float y = screenHeight / 2;
//
//    // Scaling for 4:3 in a 16:9 world
//    float aspectRatioScale = (16.0f / 9.0f) / (4.0f / 3.0f);
//
//    // Apply aspect ratio scale to X component
//    x += 0.5f * out.x * screenWidth * aspectRatioScale + 0.5f;
//    y -= 0.5f * out.y * screenHeight + 0.5f;
//
//    out.x = x;
//    out.y = y;
